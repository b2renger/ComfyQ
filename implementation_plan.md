...
- [ ] **TripoSplat (image → Gaussian splat)** — *supersedes the dropped Hunyuan3D row.* Reference workflow exported at [workflows/exported_api_workflow_comfy/3d_triposplat_image_to_gaussian_splat_api.json](workflows/exported_api_workflow_comfy/3d_triposplat_image_to_gaussian_splat_api.json). Inherently **multi-output**: `SaveVideo` (orbit render of the splat) + `SaveGLB` ×2 (`SplatToMesh` extracted mesh, plus a `SplatToFile3D` splat file). Stresses: (a) **multi-output UX** — the wire already sends full `outputs[]` but the client renders only one `result_filename` (see infra gaps below); (b) a new **Gaussian-splat viewer** component — needs a three.js splat renderer dependency, distinct from the **GLB mesh viewer already built** ([client/src/components/ui/ModelViewer.jsx](client/src/components/ui/ModelViewer.jsx), shipped during the Hunyuan exploration — reused here, *not* wasted). **Open decision when this row is built:** which splat file format to save/serve for the GS viewer (`.spz` / `.ply` / `.splat`); the exported graph routes the splat through `SaveGLB`, so a dedicated splat-save node + a new `mediaTypes` kind may be needed. Long-runtime support is already in place (see "infra READY" below).
...
- [ ] **Ideogram + Kijai prompt builder** — Ideogram-based generation driven by a Kijai prompt-builder node. Stresses: parameter surfacing for the prompt-builder node via the primitive-fallback parser (no whitelist), and possibly a richer prompt-input UX in `BookingDialog`.

**Backlog / superseded (previously queued, not in the current active set — revisit when prioritized):** Hunyuan3D 2.1 (replaced by TripoSplat for the 3D row), LTX 2.3 video-from-reference, LTX audio-driven, 360 video LoRA. The institutional notes for these (long-job polling, `ModelLifecycle /free`, audio+video multi-output, `LoraLoader` param surfacing) remain valid and several overlap with the active rows above (multi-output, audio I/O).
...
**Cross-cutting infra these rows depend on (state as of 2026-06-08):**
- **Multi-output UX — MISSING.** The wire sends the full `outputs[]` array ([server/realtime/realtimeBus.js](server/realtime/realtimeBus.js) `_toWireJob`), but every client view (Scheduler grid, `MyJobsPanel`, `ImageLightbox`) renders only the single `result_filename`. `getPrimaryDownloadFilename` ([client/src/utils/jobDisplay.js](client/src/utils/jobDisplay.js)) adds GLB-over-PNG smarts but there is no N-output gallery. Needed by TripoSplat and Qwen.
...
- **Gaussian-splat viewer — MISSING.** `.glb/.gltf` → `ModelViewer` (built). Splat formats need a dedicated three.js renderer.
