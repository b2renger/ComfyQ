...
- `workflows/image_edit_flux2_klein_9b_image_edit_ref/` — image edit with a reference image
...
- [x] Starter fixtures: three Flux2 Klein 9B workflows (`image_flux2_klein_9b_t2i`, `image_edit_flux2_klein_9b_image_edit`, `image_edit_flux2_klein_9b_image_edit_ref`) — replaced earlier `flux1_dev_t2i` smoke fixture once the Flux2 set was validated on rig.
...
- [x] **"Open in ComfyUI" → real auto-open onto the editable graph (2026-07-01 → verified 2026-07-03)** — supersedes the 2026-06-26 download-and-drag flow with a **one-click auto-open** from the admin panel (per-card button in [WorkflowSelector.jsx](client/src/components/WorkflowSelector.jsx) + the meta editor, via the shared hook [useComfyOpener.js](client/src/hooks/useComfyOpener.js)). Each bundle carries an optional **`<id>_template.json`** (the original ComfyUI **UI/litegraph** export — subgraphs/groups/notes/layout; detected but **not parsed** by [workflowRegistry.js](server/workflows/workflowRegistry.js), which also skips `_`-prefixed helper folders like `_candidate_workflows/`). **The mechanism:** ComfyUI 1.45.20 has **no URL param to open an arbitrary workflow** and its tab-restore is browser-localStorage-per-clientId (not server-writable), so we ship a **tiny frontend extension** ([server/comfyui/opener/](server/comfyui/opener/): `__init__.py` `WEB_DIRECTORY` + `js/comfyq_opener.js`) that reads a custom **`?comfyq_open=<name>`** param and `loadGraphData`s the staged workflow onto the canvas. It's **auto-installed** into `<comfy root>/custom_nodes/comfyq_opener` ([openerExtension.js](server/comfyui/openerExtension.js), version-marked) before every ComfyUI spawn; [adminCalibrator.js](server/benchmark/adminCalibrator.js) `ensureOpenerLoaded()` launches ComfyUI if down (opener present at boot → no restart) or **restarts a ComfyUI we own once** to load it (external ComfyUI → left alone → client falls back to the Workflows-sidebar handoff). New route **`POST /admin/workflows/:id/open-in-comfyui`** ([admin.js](server/routes/admin.js)) stages the template as **`<id>_template.json`** (NOT `<id>.json` — that would clobber an admin's own hand-saved workflow of the same id) into `<comfy root>/user/default/workflows/` and returns `{port, autoOpen, openName}`; the hook opens `<comfyui>/?comfyq_open=<openName>` when `autoOpen`. **Two load-correctness fixes were the crux:** (1) the load must match ComfyUI's own template loader — **`app.loadGraphData(graph, true, true, name, {openSource:'template'})`**; the 4th/5th args register **subgraph definitions**, without which a subgraph workflow (most of ours) loads **broken/unrunnable**; (2) a **canvas-mount race** (`getCanvas: canvas is null` in `loadGraphData→fitView`) because extension `setup()` runs before ComfyUI's Vue `<GraphCanvas>` mounts — fixed by `waitForCanvas()` (poll up to ~20 s for the real `<canvas>` DOM-connected + laid out), running the load **fire-and-forget after `setup()` returns**, `loadWithRetry()` (4× retry, then `restore_view:false` fallback), and swallowing all errors so no "Loading aborted" dialog. **Operational gotcha:** ComfyUI serves the extension JS from disk per request (no ComfyUI restart needed on a bump), but the **browser HTTP-caches** the module — after an `OPENER_VERSION` bump, **hard-refresh the ComfyUI tab (Ctrl+Shift+R)** to drop the stale JS (end users load it once, so this only bites the dev loop). The sibling per-card **"Activate & serve"** button (`activate(id)` → student mode) launches the API version ComfyQ Discovery serves. Template staging is display/round-trip only — the api.json still runs jobs. ✅ **Verified on the rig: a subgraph workflow auto-opens intact + runnable.**
- [x] **ComfyUI backend: Restart button + LAN toggle moved here + external "take over" (2026-07-05)** — the admin *ComfyUI backend* card ([AdminConfig.jsx](client/src/pages/AdminConfig.jsx)) gains a **Restart ComfyUI** button and now hosts the **"Expose ComfyUI to the LAN"** toggle (moved out of the Settings/paths card; saves immediately via `PUT /admin/comfy {lan_access}`, like the federation toggle). New route `POST /admin/comfyui/restart` → **`AdminCalibrator.restartBackend()`** ([adminCalibrator.js](server/benchmark/adminCalibrator.js)) restarts the ComfyUI we own reading config **fresh**, with the bind following the saved `lan_access` (the toggle is the source of truth; "Launch/Bind on network" also persists `lan_access:true` so a later restart stays network-bound instead of dropping to localhost — `bindHost = lan_access ? '0.0.0.0' : host`, and `_workerNetwork`/the `networkBound` badge must match the actual bind). **External-ComfyUI "take over" (owner-chosen):** when the running ComfyUI is external (started outside ComfyQ), Restart force-kills whatever is listening on the port — new **`killProcessOnPort()`** ([comfyProcess.js](server/workers/comfyProcess.js): Windows `netstat -ano`→`taskkill /F /T`, POSIX `lsof`→SIGKILL), then `_waitForPortFree()`, then spawns our own — leaving ComfyQ managing it afterward; gated behind a **confirm modal** since it closes the admin's manually-started instance.
- [x] **Mask painter → near-fullscreen modal (2026-07-05)** — [MaskDrawField.jsx](client/src/components/capture/MaskDrawField.jsx) (the `mask` widget) moved painting from a cramped `48vh` inline canvas into a **`createPortal`-to-`document.body` fullscreen modal** (escapes the booking dialog's transform/stacking context — the shared [Modal](client/src/components/ui/Modal.jsx) uses a `zoom-in-95` transform that would otherwise confine a nested `fixed` overlay; `z-[100]` over the dialog's `z-50`). The canvas fills the viewport preserving aspect ratio; added a **live brush-outline cursor** (drawn each `redraw()` from `brushRef`/`hoverPt`), a size readout, a wider 4–200 px range, and a small composited preview + "Paint/Edit mask" button in the form (opened on upload, reopenable). **Export byte-identical** — `commit()` (`destination-out` composite), `strokeTo`, the offscreen `maskRef` geometry, and the `onChange`/`onRemove` props are unchanged, so the `/upload`→materializer→`LoadImage` pipeline and every consumer are untouched; the shared `DynamicParamFields` means the editor "Student preview" gets the bigger editor too.
- [x] **Alphabetical workflow library (2026-07-05)** — [WorkflowSelector.jsx](client/src/components/WorkflowSelector.jsx) now sorts the available workflows by display name (`localeCompare`, case-insensitive + natural-numeric) as the base for the group filter + search, so the admin library reads predictably instead of in registry-discovery order.
...
- [x] **Flux.2 Dev image editing (kontext-style)** — *off-queue; the **first candidate-folder promotion** from `workflows/_candidate_workflows/`.* **Verified on rig (2026-07-05).** At [workflows/image_edit_flux2_dev_image_edit/](workflows/image_edit_flux2_dev_image_edit/) (source: `_candidate_workflows/Flux2_dev_image_edit_{api,template}.json`). Source image (`46 LoadImage`) → 1 MP scale (`45 ImageScaleToTotalPixels`) → `VAEEncode` (`68:44`) → **`ReferenceLatent`** (`68:43`) alongside the prompt (`68:6 CLIPTextEncode` → `68:26 FluxGuidance` 4) → `SamplerCustomAdvanced` (`68:13`) → `VAEDecode` → `9 SaveImage`; output dims follow the input aspect (`68:72 GetImageSize`). Flux.2 Dev uses embedded guidance → **single** CLIPTextEncode (no negative/CFG), unlike the Klein edit sibling. A **"Fast mode" `PrimitiveBoolean`** (`68:94`) drives **two** `ComfySwitchNode`s at once — model (`68:92`: base vs `68:89` Turbo-LoRA) and steps (`68:93`: 20 vs 8). **Zero server code, verbatim api.json** — the export was already clean (24 nodes, all reachable from `SaveImage`, no preview/dead nodes → no pruning). Exposed: source image / edit instruction / Fast mode / seed. `image-edit`→Image group; 4 models (UNET `flux2_dev_fp8mixed`, VAE `full_encoder_small_decoder`, CLIP `mistral_3_small_flux2_bf16`, LoRA `Flux_2-Turbo-LoRA_comfyui`); `minVRAM 24`. **Calibration:** cache-immune (`68:25 RandomNoise.noise_seed` literal → randomized), image auto-resolved from assets; `warmupParams` sets only a short prompt and **leaves Fast mode off**, so the gauge measures the default path — confirmed by `runtime.json` (`steps:20`, warm ≈45 s / cold ≈203 s incl. ≈158 s model load, RTX 5090). Named to match the `image_edit_flux2_klein_9b_image_edit` sibling.

- [~] **Flux.2 Dev text-to-image** — *off-queue; the t2i sibling of the Dev image-edit, 2nd candidate promotion.* **Registered 2026-07-05; pending rig verification.** At [workflows/image_flux2_dev_t2i/](workflows/image_flux2_dev_t2i/) (source: `_candidate_workflows/Flux2_dev_t2i_{api,template}.json`). Pure text→image Flux.2 Dev: prompt (`98:6 CLIPTextEncode` → `98:26 FluxGuidance` 4) → `SamplerCustomAdvanced` over `98:47 EmptyFlux2LatentImage` → `VAEDecode` → `9 SaveImage`; single CLIPTextEncode (embedded guidance, no negative/CFG). Same **"Fast mode"** `PrimitiveBoolean` (`98:104`) driving two `ComfySwitchNode`s (model base↔Turbo-LoRA, steps 20↔8). **★ One graph edit (not verbatim):** the flattened export left `width`/`height` as **separate literal 1024s on two nodes** (`98:47` EmptyLatent + `98:48` Flux2Scheduler), which one exposed param can't keep in sync (and the scheduler's resolution must match the latent's), so two `PrimitiveInt` source nodes (`98:200` Width / `98:201` Height) were added and **both** consumers rewired to them — mirroring the verified `image_flux2_klein_9b_t2i` (`75:68`/`75:69` → `75:66`+`75:62`); built programmatically (add nodes + rewire). Exposed: prompt / width / height / Fast mode / seed. `t2i`→Image group; 4 models (UNET `flux2_dev_fp8mixed`, VAE `full_encoder_small_decoder`, CLIP `mistral_3_small_flux2_fp8` — the **fp8** CLIP, vs the edit sibling's bf16; both present on the rig, LoRA `Flux_2-Turbo-LoRA_comfyui`); `minVRAM 24`; cache-immune (`98:25 RandomNoise.noise_seed`), warmup leaves Fast mode off. Validated off-rig (schema, param→node/field resolution, 21/21 reachable, width+height share one source, registry load + `hasTemplate`). **Rig-watch:** confirm width/height actually change output size, and the fp8 CLIP loads.
...
```jsonc
{
  "workflowId": "image_edit_bernini_r_image_editing",   // or omit → active workflow
