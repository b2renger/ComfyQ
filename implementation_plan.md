...
- **Audio I/O — PARTIAL (output done, input missing).** Output: server classifies `.mp3`/`.wav`/etc. and the client now has [AudioPlayer.jsx](client/src/components/ui/AudioPlayer.jsx) (`isAudio` → styled `<audio>` in cards + lightbox), built 2026-06-11 for Stable Audio 3. Input: still no `inputUploader` audio kind or `MediaCaptureField` audio type — needed only if an audio-conditioned (ACE-style) workflow lands.
